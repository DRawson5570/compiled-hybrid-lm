...
CMI has three related but distinct capability tracks. They share cartridge and
routing vocabulary, but they should not be presented as the same mechanism.

1. **Frozen base with routed runtime cartridges.** A pretrained model remains
  frozen. External cartridge artifacts are mounted at runtime, and a learned
  router selects which cartridge to activate for the prompt or task. This is
  the modular rack path validated with frozen Qwen: cartridges can be inspected,
  replaced, disabled, or hot-swapped without retraining the base model.
  Production composition is gated or routed, not blindly all-active.

2. **Training-time integrated cartridges.** Steering surfaces, compiled
  channels, adapters, or cartridge-like modules are trained with the model or
  baked into the model artifact during creation/refinement. This is the native
  model path for our own models: less hot-swap modularity, but faster runtime,
  simpler deployment, and deeper integration with the model's learned dynamics.

3. **Agentic tooling and skill loading.** The model or surrounding agent loop
  decides when a capability is needed, retrieves or loads the relevant tool,
  skill, cartridge, or artifact, uses it, and can release it afterward. This is
  the open-ended control-plane layer for large capability libraries, not a fixed
  router over one pre-mounted rack.

In short: track 1 is runtime routed activation, track 2 is training-time
integration, and track 3 is agentic discovery and use. Every benchmark or product
claim should state which track it exercises.
...
## Development Track: Learned Cartridge Router (LoRA/QLoRA on Existing Models)

*Added by Qwen3.6 at 2026-05-25 19:40 local*

*Validated on pe3 at 2026-05-25 local: both the learned-router cartridge rack and the baked native-LoRA deployment path work on `Qwen/Qwen2.5-1.5B`.*

### Target Models

Qwen, Llama, Mistral, and other HuggingFace models — not just our custom DeepSeekForCausalLM. The router is model-agnostic: any model with `nn.Linear` layers where cartridge hooks can register.

### Pipeline

```
Qwen base (frozen or LoRA/QLoRA adapted)
  -> optional general SFT / co-training pass
  -> freeze base
  -> train domain cartridges (superposition steering)
  -> train learned router (gates which cartridges fire)
  -> deploy mounted cartridges with gated activation
```

### Frozen Base Track (Current)

```
DeepSeekForCausalLM (custom, our architecture)
  -> co-train base + general steerer (compiled priors)
  -> freeze base (or mostly freeze)
  -> train domain-specific cartridges on top
  -> hot-swap cartridges at runtime
```

### Two Tracks

| | Modular Learned-Router Track | Baked Native-LoRA Track |
|---|---|---|
| **Target models** | Qwen, Llama, Mistral, etc. | Qwen, Llama, Mistral, etc. |
| **Base weights** | Frozen | Frozen |
| **What trains** | Individual cartridges plus a small router head | Native LoRA matrices |
| **Router training** | Frozen-Qwen prompt embeddings -> linear head | None at runtime; behavior is distilled into one adapter |
| **Runtime routing** | Learned gated activation, then `chain` composition | No runtime router |
| **Deployment** | Hot-swap cartridges, inspect routing, replace modules independently | One reloadable adapter artifact |
| **Validated artifact** | `learned_router/qwen_learned_router.pt` | `baked_lora_native_300/best_adapter` |

### Why LoRA/QLoRA for External Models

Our custom DeepSeekForCausalLM co-trains with the steerer from scratch — the model learns to accept and use activation offsets during its formative training. The gradient flows freely from output to steerer hooks through the co-adapted base model.

External models like Qwen were trained without a steerer. Their layers never learned to process activation offsets. Freeze Qwen + attach steerer = gradient must propagate through 28+ frozen layers to reach the hooks — attenuated to effectively zero. This was proven experimentally across 5 independent configurations (see self-improvement-research/FAILURE_ANALYSIS.md).

LoRA/QLoRA solves this by adding small trainable adapters in parallel with frozen layers. These adapters act as **signal bridges** — they conduct the steerer's gradient through the frozen model without modifying the original weights:

```
Without LoRA:  steerer → 28 frozen layers → gradient lost → no learning
With LoRA:     steerer → LoRA bridges at each layer → gradient preserved → cartridge trains
```

The LoRA adapters don't need to be large — they just need to provide a low-resistance path for the steerer's gradient. This is independent of model size: even a 1.5B model needs LoRA if it wasn't co-trained with a steerer. A 7B model with LoRA would train cartridges just as effectively as our custom 700M model without it.

### Router Architecture

The validated Qwen router is a learned control-plane head, not keyword routing. It uses frozen Qwen itself as the prompt encoder:

```
prompt
  -> frozen Qwen final hidden states
  -> mean-pooled prompt embedding
  -> trained linear router head
  -> selected cartridge_id
  -> activate selected cartridge only
  -> gated-chain generation
```

The artifact type is `qwen_embedding_linear_v1`. It stores the mounted cartridge IDs, a linear head state, confidence/margin thresholds, and metadata. Qwen's base weights do not update during router training.

The deterministic prompt router still exists as a fallback/proof harness, but it is not the product route. Product evaluations should pass `--router-path artifacts/qwen_cartridge_rack_full_20260525_171513/learned_router/qwen_learned_router.pt` and use `--composition-mode gated-chain`.

Why this matters: naive all-active task composition is unsafe. It was tested and caused cartridge interference, including private_facts collapse to 0/60. The learned router is what makes the rack scalable: all cartridges can be mounted, but only the selected cartridge is active for a prompt.

### Validated Results

**Learned-router cartridge rack:** pe3 evaluation with `learned_router_gated_chain_eval.json` passed the suite without saved-score regressions:
...
**Baked native LoRA:** pe3 `baked_lora_native_300` trained Qwen with frozen base weights and native LoRA adapters, avoiding the broken PEFT/bitsandbytes path on M40. The run reached eval_loss 0.0739 and bounded final generation eval 34/40, with heldout 3/4. The adapter is reloadable via `QwenBakedLoraRunner.from_adapter(...)` and saves `adapter.pt` plus `adapter_config.json`.

These are complementary, not competing, outputs. The learned-router rack is the product architecture for modular cartridges. The baked LoRA path is a fused artifact for cases where runtime cartridge control is unnecessary.

### Next Steps

1. **Run the full 164-row generation eval for baked LoRA** when wall-clock budget allows; the bounded 40-row eval is already positive, but the full report is the release gate.
2. **Scale the learned-router rack to more domains**: code, medical, legal, chat, and safety cartridges with the same frozen-embedding router interface.
3. **Train confidence/defer behavior**: use `confidence_threshold` and `ambiguous_margin` to allow the router to decline activation when no mounted cartridge is appropriate.
4. **Scale to 7B**: apply the same learned-router and baked-adapter paths to larger external models where cartridge overhead is negligible.
