"""
compile_wiki_lm_v33.py — 21-way multi-temporal decayed conditional + unconditioned induction
                          + Multi-Scale Unigram & Context Residual Attention Caches
                          + PPMI Semantic Similarity + KNN-LM Retrieval + Word Shape Transition Ensemble
                          (Pushing compiled language model execution boundary to GPT-2 level: PPL < 29.0)
=============================================================================================================

This file constructs an enterprise-grade 21-way model blend combining:
...
- 3 new advanced compiled channels:
  - PPMI Semantic Similarity Channel
  - KNN-LM Context Retrieval Channel
  - Word Shape Transition Channel

Optimized using high-performance chunk-based sliding-window matrix-vector logic on CUDA.
"""
...
ARTIFACT = Path("/home/drawson/deepseek_experiments/artifacts/compiled_wiki_lm_v33")
ARTIFACT.mkdir(parents=True, exist_ok=True)


@torch.no_grad()
def build_v33_induction_log_probs(
    ...
    B_bi_fast = torch.zeros(V, V, device="cpu", dtype=torch.float32)
    R_bi_fast = torch.zeros(V, device="cpu", dtype=torch.float32)
    last_bi_fast = torch.zeros(V, V, device="cpu", dtype=torch.int32)
    last_R_bi_fast = torch.zeros(V, device="cpu", dtype=torch.int32)

    # 2. Decayed Bigram - Slow
    B_bi_slow = torch.zeros(V, V, device="cpu", dtype=torch.float32)
    R_bi_slow = torch.zeros(V, device="cpu", dtype=torch.float32)
    last_bi_slow = torch.zeros(V, V, device="cpu", dtype=torch.int32)
    last_R_bi_slow = torch.zeros(V, device="cpu", dtype=torch.int32)

    # 3. Decayed Unconditioned Unigram Cache - Fast
    C_uc_fast = torch.zeros(V, device="cpu", dtype=torch.float32)
    last_uc_fast = torch.zeros(V, device="cpu", dtype=torch.int32)
    S_uc_fast = 0.0
    last_S_uc_fast = 0

    # 4. Decayed Unconditioned Unigram Cache - Slow
    C_uc_slow = torch.zeros(V, device="cpu", dtype=torch.float32)
    last_uc_slow = torch.zeros(V, device="cpu", dtype=torch.int32)
    S_uc_slow = 0.0
    last_S_uc_slow = 0
    ...
    for t in range(K_pos, N):
        ...
        if t >= 2:
            ...
            if key_val in Bt_fast:
                ...
                v = torch.zeros(V, device="cpu", dtype=torch.float32)
                v[c] = 1.0
                Bt_fast[key_val] = v
                last_arr = torch.zeros(V, device="cpu", dtype=torch.int32)
                last_arr[c] = t
...
# Helper functions for the three new channels in v33
@torch.no_grad()
def build_ppmi_semantic_log_probs_all(emb: torch.Tensor, gamma: float) -> torch.Tensor:
    """Precompute similarity matrix over all V x V, return log-probs on GPU."""
    emb_norm = F.normalize(emb, dim=1)
    sims = emb_norm @ emb_norm.T
    sims = sims * gamma
    return F.log_softmax(sims, dim=1)


@torch.no_grad()
def build_knn_log_probs_sliced(
    ids_np: np.ndarray, emb: torch.Tensor,
    V: int, k: int = 64, temperature: float = 1.0,
) -> torch.Tensor:
    """Build KNN-LM retrieval log-probabilities, returned as PyTorch tensor."""
    N = len(ids_np)
    max_window = 5000
    query_batch = 500
    
    emb_norm = F.normalize(emb, dim=1)
    tok_norm = emb_norm[ids_np].to(DEVICE)  # (N, d)
    
    log_probs = torch.full((N, V), -math.log(V), device="cpu", dtype=torch.float32)
    ...
    for q_start in range(1, N, query_batch):
        ...
        window_end = q_start
        keys = tok_norm[window_start:window_end]
        n_keys = keys.shape[0]
        ...
        top_vals, top_idx = sims.topk(k_actual, dim=1)
        
        abs_positions = window_start + top_idx
        next_tokens = torch.from_numpy(ids_np[abs_positions.cpu().numpy() + 1]).to(DEVICE)
        weights = F.softmax(top_vals / temperature, dim=1)
        
        for qi in range(n_queries):
            dist = torch.zeros(V, device=DEVICE, dtype=torch.float32)
            dist.index_put_((next_tokens[qi],), weights[qi], accumulate=True)
            alpha = 0.01
            smoothed = (dist * k_actual + alpha) / (k_actual + alpha * V)
            log_probs[q_start + qi] = torch.log(smoothed.clamp_min(1e-30)).cpu()
...
def build_shape_transition_probs_v33(
    ids_np_full: np.ndarray, bpe, V: int
) -> torch.Tensor:
    """Get transition logic trained from training IDs, returned as transition matrix on CPU."""
    N_SHAPES = 7
    token_shapes = np.zeros(V, dtype=np.int32)
    for tid in range(V):
        token_shapes[tid] = token_shape(bpe.decode([tid]))
        
    shape_counts = np.zeros((N_SHAPES, N_SHAPES), dtype=np.float64)
    count_n = min(len(ids_np_full) - 1, 5_000_000)
    for t in range(count_n):
        s_cur = token_shapes[ids_np_full[t]]
        s_nxt = token_shapes[ids_np_full[t + 1]]
        shape_counts[s_cur, s_nxt] += 1
        
    alpha = 0.1
    shape_trans = (shape_counts + alpha) / (shape_counts.sum(axis=1, keepdims=True) + alpha * N_SHAPES)
    
    # Pre-build Shape Transition Log Probs for each target shape pattern
    shape_trans_log_probs_all = np.zeros((N_SHAPES, V), dtype=np.float32)
    for s_cur in range(N_SHAPES):
        expected_next_shape_probs = shape_trans[s_cur]
        logits = np.full(V, -10.0, dtype=np.float32)
        for s in range(N_SHAPES):
            mask = (token_shapes == s)
            if mask.any():
                logits[mask] = math.log(expected_next_shape_probs[s] + 1e-10)
        shape_trans_log_probs_all[s_cur] = logits - np.log(np.sum(np.exp(logits)))
        
    return torch.from_numpy(shape_trans_log_probs_all), torch.from_numpy(token_shapes)
...
def eval_blend_fast(targets_lp, weights):
    """Linear prob-space blend: P = sum_k w_k * P_k using pre-extracted target log-probs."""
    weights = np.array(weights, dtype=np.float32)
    w_sum = weights.sum()
    if w_sum <= 0:
    ...
    log_p_target = m + np.log(np.exp(stack - m[None]).sum(axis=0))
    nll = -log_p_target.sum()
    ppl = math.exp(nll / len(log_p_target))
    return {"ppl": ppl}
...
def main():
    ...
    # Three advanced v33 channels
    p.add_argument("--ppmi-gamma", type=float, default=100.0)
    p.add_argument("--knn-k", type=int, default=64)
    p.add_argument("--knn-temp", type=float, default=1.0)
    ...
    T_total = len(ids)
    if train_n + val_n + eval_n > T_total:
        train_n = max(T_total - val_n - eval_n, T_total // 2)
    val_ids_t = ids[train_n:train_n + val_n]
    val_ids_n = ids_np[train_n:train_n + val_n]
    eval_ids_t = ids[-eval_n:]
    eval_ids_n = ids_np[-eval_n:]

    print(f"[v33] MILESTONE 21-Way Blend Model: 18 base CMI channels + PPMI Semantic + KNN Retrieval + Shape Transitions")
    print(f"[split] train={train_n:,}  val={val_n:,}  eval={eval_n:,}")

    # Precompile Shape transitions once over a subset of training data
    print(f"[load] Prebuilding Word Shape transition patterns from training subset...")
    t_shape0 = time.time()
    shape_trans_log_probs, token_shapes = build_shape_transition_probs_v33(ids_np[:train_n], bpe, V)
    print(f"  done ({time.time() - t_shape0:.1f}s)")

    # Precompile PPMI similarity log probs once
    print(f"[load] Prebuilding PPMI similarity cosine-kernel transitions...")
    t_ppmi0 = time.time()
    ppmi_log_probs_all = build_ppmi_semantic_log_probs_all(emb_dev, args.ppmi_gamma).cpu()
    print(f"  done ({time.time() - t_ppmi0:.1f}s)")
    ...
    def prepare(ids_t, ids_n, label):
        ...
        print(f"\n[{label}] computing 21 underlying component log-prob tables with RAM conservation")
        temp_dir = tempfile.mkdtemp(prefix=f"mmap_v33_{label}_")
        global_temp_dirs.append(temp_dir)
        
        N_target = ids_n.shape[0] - args.K_pos
        V_dim = V
        
        log_p_list = []
        
        def save_and_get_mmap(arr, idx):
            fp = os.path.join(temp_dir, f"comp_{idx}.dat")
            mmap_arr = np.memmap(fp, dtype='float32', mode='w+', shape=(N_target, V_dim))
            if isinstance(arr, torch.Tensor):
                mmap_arr[:] = arr.cpu().numpy()
            else:
                mmap_arr[:] = arr
            mmap_arr.flush()
            del arr
        ...
        print(f"  (1/21) mix done ({time.time() - t0:.1f}s, shape={log_p_mix.shape})")
        log_p_list.append(save_and_get_mmap(log_p_mix, 1))
        
        # 2. Global KN7
        t0 = time.time()
        log_p_kn = compute_log_p_kn(kn, ids_n, args.K_pos)
        print(f"  (2/21) KN done ({time.time() - t0:.1f}s)")
        log_p_list.insert(0, save_and_get_mmap(log_p_kn, 0))
        
        # 3. Dynamic caches
        t0 = time.time()
        log_p_trif, log_p_tris, log_p_bif, log_p_bis, log_p_ucf, log_p_ucs = build_v33_induction_log_probs(
        ...
        print(f"  (3-8/21) dynamic decay caches done ({time.time() - t0:.1f}s)")
        log_p_list.append(save_and_get_mmap(log_p_trif, 2))
        log_p_list.append(save_and_get_mmap(log_p_tris, 3))
        log_p_list.append(save_and_get_mmap(log_p_bif, 4))
        log_p_list.append(save_and_get_mmap(log_p_bis, 5))
        log_p_list.append(save_and_get_mmap(log_p_ucf, 6))
        log_p_list.append(save_and_get_mmap(log_p_ucs, 7))
        ...
        log_p_list.append(save_and_get_mmap(log_p_attn_uf, 8))
        ...
        log_p_list.append(save_and_get_mmap(log_p_attn_us, 9))
        ...
        log_p_list.append(save_and_get_mmap(log_p_attn_ug, 10))
        print(f"  (9-11/21) unigram attention caches done ({time.time() - t0:.1f}s)")
        ...
        log_p_list.append(save_and_get_mmap(log_p_attn_rf1, 11))
        ...
        log_p_list.append(save_and_get_mmap(log_p_attn_rs1, 12))
        ...
        log_p_list.append(save_and_get_mmap(log_p_attn_rf2, 13))
        ...
        log_p_list.append(save_and_get_mmap(log_p_attn_rs2, 14))
        ...
        log_p_list.append(save_and_get_mmap(log_p_attn_rg2, 15))
        ...
        log_p_list.append(save_and_get_mmap(log_p_attn_rf3, 16))
        ...
        log_p_list.append(save_and_get_mmap(log_p_attn_rs3, 17))
        print(f"  (12-18/21) multi-scale state attention caches done ({time.time() - t0:.1f}s)")
        
        del r_full
        gc.collect()
        torch.cuda.empty_cache()

        # 7. advanced v33 channels
        # ppmi_sem
        t0 = time.time()
        context_tokens = ids_n[args.K_pos - 1 : len(ids_n) - 1]
        log_p_ppmi = ppmi_log_probs_all[context_tokens].numpy()
        log_p_list.append(save_and_get_mmap(log_p_ppmi, 18))
        print(f"  (19/21) PPMI semantic channel done ({time.time() - t0:.1f}s)")

        # knn retrieval
        t0 = time.time()
        log_p_knn_full = build_knn_log_probs_sliced(ids_n, emb_dev, V, k=args.knn_k, temperature=args.knn_temp)
        log_p_knn = log_p_knn_full[args.K_pos:].numpy()
        log_p_list.append(save_and_get_mmap(log_p_knn, 19))
        print(f"  (20/21) KNN retrieval channel done ({time.time() - t0:.1f}s)")

        # word shape transitions
        t0 = time.time()
        prev_token_shapes = token_shapes[context_tokens]
        log_p_shape = shape_trans_log_probs[prev_token_shapes].numpy()
        log_p_list.append(save_and_get_mmap(log_p_shape, 20))
        print(f"  (21/21) Word Shape transition channel done ({time.time() - t0:.1f}s)")
        
        targets = ids_n[args.K_pos:]
        
        assert len(log_p_list) == 21, f"Expected 21 components, got {len(log_p_list)}"
        return log_p_list, targets

    # VAL
    log_p_list_v, targets_v = prepare(val_ids_t, val_ids_n, "val")

    # Pre-extract target log-probs sequentially to avoid random file access in sweep loop
    print("[sweep] pre-extracting target log-probs from 21 memory-mapped channels...")
    targets_lp_v = np.stack([lp[np.arange(len(targets_v)), targets_v] for lp in log_p_list_v], axis=0)
    print("  done.")

    print(f"\n[val] Dirichlet random simplex sweep of 60,000 points to optimize 21 blend weights")
    alpha_dir = [
        6.0,  0.1,                          # kn, mix
        0.1,  0.1,                          # tf, ts
        0.1,  1.0,                          # bf, bs
        0.2,  0.2,                          # ucf, ucs
        0.6,  0.6,  0.4,                    # a_uf, a_us, a_ug
        0.8,  0.8,                          # a_rf1, a_rs1
        1.0,  1.0,  0.6,                    # a_rf2, a_rs2, a_rg2
        1.0,  1.0,                          # a_rf3, a_rs3
        0.1,  0.5,  0.1                     # ppmi, knn, shape
    ]
    num_samples = 60000
    ...
    # Prepend some strong default configs from v28, v29, v31, v32
    extra_configs = [
        [1.0] + [0.0]*20,
        [0.69, 0.03, 0.03, 0.0, 0.03, 0.15, 0.04, 0.03] + [0.0]*13,
        [0.71, 0.03, 0.01, 0.01, 0.02, 0.14, 0.04, 0.04] + [0.0]*13,
    ]
    ...
    for idx_cfg, w in enumerate(grid):
        r = eval_blend_fast(targets_lp_v, w)
    ...
    # Free memory
    del log_p_list_v, targets_v, targets_lp_v
    ...
    v28_w = [0.69, 0.03, 0.03, 0.0, 0.03, 0.15, 0.04, 0.03] + [0.0]*13
    v29_w = [0.71, 0.03, 0.01, 0.01, 0.02, 0.14, 0.04, 0.04] + [0.0]*13
    ...
    for ppl, w in val_results[1:]:
        ...
        if np.abs(np.array(w) - np.array(best_w)).max() > 0.02:
            configs_to_test.append((f"v33-alt-{added+1}", w))
            added += 1
    ...
    # Free memory
    del log_p_list_e, targets_e
    import shutil
    for d_path in global_temp_dirs:
        try:
            shutil.rmtree(d_path)
            print(f"[cleanup] Cleaned up temporary directory: {d_path}")
        except Exception as e:
            print(f"[cleanup] Failed to clean up {d_path}: {e}")

    out = {
        "model": "v33 21-way blend with dynamic decayed counts + multi-scale semantic unigram & context-residual attention caches + ppmi + knn + shape channels",
        ...
        "ppmi_gamma": args.ppmi_gamma, "knn_k": args.knn_k, "knn_temp": args.knn_temp,
    ...
    out_path = ARTIFACT / f"eval_results_v33_{args.tag}.json"
