...
DEFAULT_TASKS: tuple[AssistantTask, ...] = (
    ...
    AssistantTask(
        task_id='assistant_name',
        category='conversation',
        prompt='What is your name?',
        required_any=('cmi', 'assistant'),
        forbidden_any=('douglas', 'nice to meet you', 'creator'),
        min_words=4,
        max_words=60,
    ),
    AssistantTask(
        task_id='story_instruction',
        category='creative',
        prompt='Tell me a story. You choose the length and the topic.',
        required_any=('once', 'one day', 'story', 'village', 'city', 'forest', 'robot', 'child', 'star'),
        forbidden_any=('shortest useful answer', 'states the result', 'next action', 'choose the length'),
        min_words=35,
        max_words=180,
    ),
    AssistantTask(
...
def evaluate_runtime(runtime, tasks: tuple[AssistantTask, ...], args) -> list[AssistantEvalRow]:
